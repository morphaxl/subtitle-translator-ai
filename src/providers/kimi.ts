...
export function createKimiProvider(config: ProviderConfig): TranslationProvider {
  const { 
    apiKey, 
    baseUrl = PROVIDERS.kimi.baseUrl,
    model = PROVIDERS.kimi.defaultModel,
    sourceLang, 
    targetLang,
    maxRetries = 3,
    retryDelay = 1000,
...
Output exactly ${subtitles.length} translated lines:`;
  ...
  return { name: 'kimi', translateBatch, getStats, resetStats };
}
