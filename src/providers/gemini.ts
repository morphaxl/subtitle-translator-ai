...
Translate these ${subtitles.length} subtitle lines from ${sourceLang} to ${targetLang}:
...
Output exactly ${subtitles.length} translated lines:`;
    ...
    if (data.usageMetadata) {
      ...
      stats.totalTokens += data.usageMetadata.totalTokenCount || 0;
    }
  ...
  return { name: 'gemini', translateBatch, getStats, resetStats };
}
